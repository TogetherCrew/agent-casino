...
event BetBear(address indexed sender, uint256 indexed epoch, uint256 amount, string thesis);
event BetBull(address indexed sender, uint256 indexed epoch, uint256 amount, string thesis);
    ...
    constructor(
        ...
        oracle = IEOFeedAdapter(_oracleAddress);
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch, string calldata thesis) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
        require(bytes(thesis).length <= 1000, "Thesis too long!");
        // Update round data
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch,string calldata thesis) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
